...
public class TestCollector implements MetricsCollector {
    private TestSampleMBean testSampleMBean = UtilInjector.getInstance(TestSampleMBean.class);
    private boolean isSamplesUpdated = false;

    @Override
    public boolean isSupport(String tag) {
        return Objects.equals(tag, "test");
    }
    ...
    public List<MetricsSample> export() {
        ...
        CounterMetricsSample sample = new CounterMetricsSample("test",testSampleMBean.getNum());
        samples.add(sample);
        return samples;
    }
}
